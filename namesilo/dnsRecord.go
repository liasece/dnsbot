...
func sendRequest(url string) ([]byte, error) {
	...
	defer func() {
		...
		if errClose != nil {
			log.Error("resp.Body.Close() failed", log.ErrorField(errClose))
		}
	}()
...
func dnsList(domain, key string) (*ListResp, error) {
	...
	// log.Info("dnsList data %s", string(data))
	resp := &ListResp{}
	err = xml.Unmarshal(data, resp)
	if err != nil {
		return nil, err
	}
	log.Info("get dnsList", log.Reflect("resp", resp))
...
func dnsUpdate(key, domain, recordID, host, value string) error {
	...
	log.Info("dnsUpdate response", log.Reflect("resp", resp))
